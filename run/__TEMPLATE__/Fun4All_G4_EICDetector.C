...
R__ADD_INCLUDE_PATH($FUN4ALL_TOFDIR/include)
                           ...
                           )
{
  ...
  // Or:
  // Use Pythia 6
  const bool runpythia6 = do_simulation && true;
  // Or:
  // Use HEPGen
  ...
  const bool particles = do_simulation && false && !readhits;
  ...
  // Write DST file
  bool do_dst_write = do_simulation && false;
...
*/
  ...
  if (do_tofreco) {
    gSystem->Load("libtofreco.so");
    TOFReco *tofreco = new TOFReco("TOFReco", string(outputFile) + "_tofreco.root");
...
int Fun4All_G4_EICDetector_Sim(const int nEvents = 1, const char * outputFile = "G4EICDetector.root")
{
  do_simulation = true;
  do_tofreco = false;
  return Fun4All_G4_EICDetector(nEvents, "", outputFile);
}
...
int Fun4All_G4_EICDetector_Rec(const char * inputFile)
{
  do_simulation = false;
  do_tofreco = true;
  return Fun4All_G4_EICDetector(0, inputFile, "");
}
